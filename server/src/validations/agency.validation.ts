...
class AgencyValidation {
  static validateCreateAgency(body: any) {
    const schema = Joi.object({
      ...
      description: Joi.string().max(600).allow(null, "").messages({
      ...
      contactPhone: Joi.string()
        ...
        .messages({
          "phoneNumber.invalid":
            "Phone number did not seem to be a valid phone number",
        }),
    });
  ...
  static validateUpdateAgencyPaload(body: any) {
    const schema = Joi.object({
      ...
      description: Joi.string().max(600).allow(null, "").optional().messages({
        "string.max": "Description must not exceed 600 characters",
      }),
