...
const reject = (error: any) => {
    ...
    if (error.message === "timeout") {
        return showToast(
            <ToastContainer title="Timeout" description="Oh, Sorry! This is taking longer than expected. Please, try again later" />,
            "info"
        );
    }
    ...
    if (status === 500) {
        ...
        return isOnline
            ? showToast(
                <ToastContainer title="Something went wrong. Please try again later" description={`Error code: ${errorCode ?? error?.message}`} />,
                "error"
            )
            : showToast(
                <ToastContainer title="No internet connections" description="Please make sure your device is connected to the internet and try again" />,
                "warning",
            );
    }
    if (error.code == "ERR_NETWORK") {
        return showToast(
            <ToastContainer title="Network Error" description="Please check your internet connection and try again" />,
            "warning",
        );
    }
    ...
    const errorMessage = (errorMessage: any) => {
        if (status == 403 || status == 409) {
            ...
            showToast(<ToastContainer title={title} description={errorMessage} />,
                "warning");
        } else {
            showToast(
...
instance.interceptors.request.use((config: any) => {
    ...
    if (data && authRoute && process.env.NODE_ENV !== "development") {
